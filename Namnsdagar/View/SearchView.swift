...
struct SearchView: View {
    ...
    var body: some View {
        NavigationView {
            List {
                TextField(String(localized: "Enter a name"), text: $searchQuery)
                ...
                if searchResults.isEmpty && hasSearched {
                    Text(String(localized: "No results found for '\(searchQuery)'. Please try another name."))
                        .foregroundColor(.red)
                } else {
            ...
            .navigationBarTitle(String(localized: "Search"), displayMode: .inline)
        }
    }
